...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      auto circle_center_x = cur_x + CIRCLE_OFFSETS[c] * cos(cur_yaw);  // <- Update 
      auto circle_center_y = cur_y + CIRCLE_OFFSETS[c] * sin(cur_yaw); // <- Update 
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double dist = sqrt(pow((circle_center_x - actor_center_x), 2) + pow((circle_center_y - actor_center_y), 2));  // <- Update
